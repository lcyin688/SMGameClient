...
var syncTTFWords = {
    ...
    getExcelWords: function (file, allLg) {
        let allWords = '';
        let targetSheet = null;
        let excelObj = xlsx.parse(file);
        for (let one of excelObj) {
            if (one.name == '多语言|langugeActivity') {
                targetSheet = one;
                break;
            }
        }
        if (!targetSheet) {
            console.log(`dont find the sheet, named as 多语言|langugeActivity!`);
            return;
        }
        ...
        for (let i = 0; i < keys.length; i++) {
            const name = keys[i];
            if (allLg.indexOf(name) >= 0) {
                arrIdx.push(i);
            }
        }
        for (let line = 4; line < targetSheet.data.length; line++) {
            for (let oneK of arrIdx) {
                allWords += targetSheet.data[line][oneK];
            }
        }
        return allWords;
    },
    ...
    /** 获得需统计语言列表 */
    getLGList: function (prjRoot) {
        let file = prjRoot + 'assets/c2f-framework/gameCfg.json';
        ...
        let allLg = [];
        if (cfgObj) {
            allLg = cfgObj.language.type.concat([]);
        }
        return allLg;
    },

    getAllWords: function (dir, branch) {
        let cmmWords = `
        一画
        一乙
        二画
        ...
        八画
        玫卦坷坯拓坪坤拄拧拂拙拇拗茉昔苛苫苟苞茁苔枉枢枚枫杭郁矾奈奄殴歧卓昙哎咕呵咙呻咒咆咖帕账贬贮氛秉岳侠侥侣侈卑刽刹肴觅忿瓮肮肪狞庞疟疙疚卒氓炬沽沮泣泞泌沼怔怯宠宛衩祈诡帚屉弧弥陋陌函姆虱叁绅驹绊绎
        九画
        ...
        十六画
        撼擂擅蕾薛薇擎翰噩橱橙瓢蟥霍霎辙冀踱蹂蟆螃螟噪鹦黔穆篡篷篙篱儒膳鲸瘾瘸糙燎濒憾懈窿缰
        十七画
        壕藐檬檐檩檀礁磷瞭瞬瞳瞪曙蹋蟋蟀嚎赡镣魏簇儡徽爵朦臊鳄糜癌懦豁臀
        十八画
        藕藤瞻嚣鳍癞瀑襟璧戳
        十九画
        攒孽蘑藻鳖蹭蹬簸簿蟹靡癣羹
        ...
        二十四画
        矗'`;
        let cmmFlags = 'ⅰⅱⅲⅳⅴⅵⅶⅷⅸⅹⅠⅡⅢⅣⅤⅥⅦⅧⅨⅩⅪⅫ1234567890一二三四五六七八九十abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ壹贰叁肆伍陆柒捌玖拾佰仟万萬亿元圆角分零整×÷±/=∥∠≌∽≦≧≒﹤﹥≈≡≠=≤≥<>≮≯∷∶∫∮∝∞∧∨∑∏∪∩∈∵∴⊥∥∠⌒⊙√∟⊿㏒㏑%‰.，、；：？！ˉˇ¨~々～‖∶"|·…—～-〃“”〝〞’〔〕〈〉《》,.;:?!":()[]{}<>—-…/\\|&@#$%^*+-=_`~"(){}[]<>/\|&@#$%^*+-=_≠≥≤∞∑∏∫√∞∑∏∪∩∈∵∴⊥∥∠⌒⊙√∟⊿㏒㏑%‰→←↑↓↖↘';
        let allWords = cmmFlags + cmmWords;

        let srcFiles = [];
        srcFiles.push(dir + 'assets/c2f-framework/game/words.ts');
        srcFiles.push(dir + 'assets/language_tw/words.txt');
        srcFiles.push(dir + 'assets/resources/config/langugeCore.txt');
        let allLgs = this.getLGList(dir);

        //项目目录
        let dirIdx = dir.lastIndexOf('\\');
        if (dirIdx == dir.length - 1) {
            dirIdx = dir.substring(0, dir.length - 1).lastIndexOf('\\');
        }
        let rootDir = dir.substring(0, dirIdx + 1);
        let actWordsFile = `${rootDir}x1_plan/配置_${branch}/excels/langugeActivity活动多语言表.xlsx`;
        let actWords = this.getExcelWords(actWordsFile, allLgs);

        for (let one of srcFiles) {
            let dataTxt = fs.readFileSync(one, 'utf8');
            allWords += dataTxt;
        }
    ...
    modifyHtml: function (file, words, endCb) {
        this.readFileToArr(file, (arr) => {
            let content = '';
            let beginIdx = arr.findIndex((one) => {
                return one.indexOf('<div') != -1;
            });
            let endIdx = arr.findIndex((one) => {
                return one.indexOf('</div>') != -1;
            });
            ...
            for (let i = 0; i < arr.length; i++) {
                ...
                if (i < beginIdx || i > endIdx) {
                    ...
                    content += `<div class="fzcsjt">${words}</div>\n`;
                }
            }
    ...
    doWork: function (dir, branch) {
        dir = dir || 'D:/1_work/X1/x1_client/';
        let allWords = this.getAllWords(dir, branch);

        let htmlFile = dir + 'tools/fontspider/demo/font-awesome/demo.html';
        this.modifyHtml(htmlFile, allWords, null);
    },
}
