...
var genSceneAtlas = {
    ...
    getExcelData: function () {
        let file = `${this.planDir}/excels/scene场景.xlsx`;
        ...
        if (!targetSheet) {
            console.log(`dont find the sheet, named as 场景裁剪!`);
            return;
        }
        let allCfg = {};
...
<plist version="1.0">
    ...
    doWork: function (dir, clientFolder) {
        ...
        if (dir) {
            ...
            if (find >= 0) {
                this.outDir = dir.substring(0, find - 1) + `\\${clientFolder}\\assets\\mainPack\\image\\ui\\batScene\\`
            } else {
                console.log('progect path is valid')
            }
        } else {
            this.outDir = 'D:/1_work/X1/x1_client/assets/mainPack/image/ui/batScene/'
        }
        console.log('planDir', this.planDir);
...
if (process.argv.length > 2) {
    ...
    genSceneAtlas.doWork(dir, clientF);
} else {
