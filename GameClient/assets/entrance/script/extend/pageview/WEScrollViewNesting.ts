...
@ccclass
@menu('c2f/scroll/WEScrollViewNesting(支持嵌套的滑动 cc.ScrollView)')
export class WEScrollViewNesting extends cc.ScrollView {
    /**
     * 记录起始点
     */
    ...
    protected _onTouchMoved(event: cc.Event.EventTouch, captureListeners?: cc.Node[]): void {
        ...
        if (this.node === scrollTarget) {
            ...
            // 联动 WEPageView
            let page = scrollTarget.getComponent(c2f.ui.WEPageView);
            if (page) {
                page.onTouchMove();
            }
        }
