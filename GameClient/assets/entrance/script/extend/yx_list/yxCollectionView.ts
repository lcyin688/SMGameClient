import { WEBatchItem } from '../render/WEBatchItem';
import { YXIndexPath } from './yxTypes';
...
declare global {
    interface IUI {
         ...
         * 列表组件
         */
        YXCollectionView: typeof YXCollectionView;
    }
    ...
    namespace c2f {
        namespace ui {
             ...
             * 列表组件
             */
            type YXCollectionView = InstanceType<typeof YXCollectionView>;
        }
    }
}
 ...
 */
class _cell_ extends cc.Component {
    /**
     * 此节点是通过哪个标识符创建的
     */
    identifier: string;
 ...
 * 私有组件
 * 内部滚动视图组件
 * https://github.com/cocos/cocos-engine/blob/v2.4.14/cocos2d/core/components/CCScrollView.js
 */
class yxscroll_view extends cc.ScrollView {
    /**
     * 禁掉鼠标滚轮
     */
    ...
    _yxOnStartInertiaScroll: (touchMoveVelocity: cc.Vec3) => void;
    ...
    hasNestedViewGroup(event?: Event, captureListeners?: Node[]): boolean {
     ...
     * 获取本次滑动是要滑动哪个列表
     */
    _yxGetScrollTarget(event: cc.Event.EventTouch, captureListeners?: cc.Node[]): cc.Node {
        // 尝试获取本次已经确定了的滚动节点
        ...
        for (let index = 0; index < nodes.length; index++) {
            const element = nodes[index];
            let scrollComp = element.getComponent(yxscroll_view);
            if (scrollComp) {
                let collectionView = element.getComponent(YXCollectionView);
                if (collectionView && collectionView.scrollEnabled == false) {
                    continue;
                } // 不支持滚动
                if (result == null) {
                    result = element;
 ...
 */
export interface YXCollectionViewCell extends cc.Component {
    unuse(): void;
    reuse(args: any): void;
}
...
@ccclass
@disallowMultiple
@menu('c2f/UI/YXCollectionView')
export class YXCollectionView extends cc.Component {
    /**
     * 版本号
     */
    ...
    get scrollView(): cc.ScrollView {
        let result = this.node.getComponent(yxscroll_view);
        if (result == null) {
            result = this.node.addComponent(yxscroll_view);
            // 配置 scroll view 默认参数
            result.brake = 0.6;
        }
        if (result.content == null) {
            let content = new cc.Node(`yx.scroll.content`);
            if (this.bBatchItem) {
                content.addComponent(WEBatchItem);
            }
        ...
        if (this.mask) {
            let mask = result.node.getComponent(cc.Mask);
            if (mask == null) {
    ...
    private get _scrollView(): yxscroll_view {
        return this.scrollView as unknown as yxscroll_view;
    }
     ...
     * 允许手势滚动
     */
    @property({ tooltip: `是否开启自动分层和批` })
    bBatchItem: boolean = false;
     ...
     */
    register(identifier: string, maker: () => cc.Node, poolComp: (new (...args: any[]) => YXCollectionViewCell) | null = null) {
        let pool = new cc.NodePool(poolComp);
        this.pools.set(identifier, pool);
        this.makers.set(identifier, maker);
    }
    private pools: Map<string, cc.NodePool> = new Map();
    private makers: Map<string, () => cc.Node> = new Map();
    ...
    dequeueReusableCell(identifier: string): cc.Node {
        ...
        if (result == null) {
            ...
            if (result instanceof cc.Node && pool.poolHandlerComp instanceof Function) {
                result.getComponent(pool.poolHandlerComp) || result.addComponent(pool.poolHandlerComp as any);
            }
     ...
     */
    cellForItemAt: (indexPath: YXIndexPath, collectionView: YXCollectionView) => cc.Node = null;
     ...
     */
    onCellDisplay: (cell: cc.Node, indexPath: YXIndexPath, collectionView: YXCollectionView) => void = null;

    /**
     * 点击到节点后调用
     */
    onTouchItemAt: (indexPath: YXIndexPath, collectionView: YXCollectionView) => void = null;
    private onTouchItem(ev: cc.Event.EventTouch) {
     ...
     * @param indexPath
     */
    getVisibleNode(indexPath: YXIndexPath): cc.Node | null {
     ...
     * @returns
     */
    getVisibleNodeIndexPath(node: cc.Node): YXIndexPath {
     ...
     * 回收不可见节点
     */
    private recycleInvisibleNodes(visibleRect: cc.Rect = null) {
        if (visibleRect == null) {
     ...
     */
    private applyLayoutAttributes(node: cc.Node, attributes: YXLayoutAttributes, bringToFront: boolean) {
        let cell = node.getComponent(_cell_);
        ...
        let pos = attributes[`_in_collection_view_pos`] as cc.Vec3;
        if (pos == null) {
            pos = node.position.clone() as cc.Vec3;
    ...
    scrollTo(indexPath: YXIndexPath, timeInSecond: number = 0, attenuated: boolean = true) {
        let toOffSet: cc.Vec2 = this.layout.scrollTo(indexPath, this);
        if (toOffSet == null) {
    ...
    private _scroll_offset_on_touch_start: cc.Vec2 = null;
    private onStartInertiaScroll(touchMoveVelocity: cc.Vec3) {
...
export default YXCollectionView;

c2f.ui.YXCollectionView = YXCollectionView;
