import { YXCollectionView, YXLayout, YXLayoutAttributes } from './yxCollectionView';
...
declare global {
    interface IUI {
         ...
         * 列表组件
         */
        YXFlowLayout: typeof YXFlowLayout;
    }
    ...
    namespace c2f {
        namespace ui {
             ...
             * 列表组件
             */
            type YXFlowLayout = InstanceType<typeof YXFlowLayout>;
        }
    }
}
...
export class YXFlowLayout extends YXLayout {
     ...
     * 元素大小
     */
    itemSize: cc.Size | ((indexPath: YXIndexPath, layout: YXFlowLayout, collectionView: YXCollectionView) => cc.Size) = new cc.Size(100, 100);
    getItemSize(): cc.Size {
        if (this.itemSize instanceof Function == false) {
            return this.itemSize as cc.Size;
        }
    ...
    getVerticalSpacing(): number {
        if (this.verticalSpacing instanceof Function == false) {
            return this.verticalSpacing as number;
        }
        throw new Error('YXFlowLayout: 动态配置的布局参数不支持直接获取，请检查自己的布局逻辑并谨慎的通过动态配置自己获取，注意避免死循环');
    }
    ...
    getHorizontalSpacing(): number {
        if (this.horizontalSpacing instanceof Function == false) {
            return this.horizontalSpacing as number;
        }
        throw new Error('YXFlowLayout: 动态配置的布局参数不支持直接获取，请检查自己的布局逻辑并谨慎的通过动态配置自己获取，注意避免死循环');
    }
    ...
    getSectionInset(): YXEdgeInsets {
        if (this.sectionInset instanceof Function == false) {
            return this.sectionInset as YXEdgeInsets;
        }
        throw new Error('YXFlowLayout: 动态配置的布局参数不支持直接获取，请检查自己的布局逻辑并谨慎的通过动态配置自己获取，注意避免死循环');
    }
    ...
    targetOffset(collectionView: YXCollectionView, touchMoveVelocity: cc.Vec3, startOffset: cc.Vec2): { offset: cc.Vec2; time: number } {
        ...
        let threshold = 0.2;
        if (this.scrollDirection == YXFlowLayout.ScrollDirection.HORIZONTAL) {
            let idx = Math.round(offset.x / collectionView.scrollView.node.width);
            let r = touchMoveVelocity.x / collectionView.scrollView.node.width;
            if (startOffset && Math.abs(r) >= threshold) {
                idx = Math.round(startOffset.x / collectionView.scrollView.node.width) + (r > 0 ? -1 : 1);
            }
            offset.x = idx * collectionView.scrollView.node.width;
        }
        if (this.scrollDirection == YXFlowLayout.ScrollDirection.VERTICAL) {
            let idx = Math.round(offset.y / collectionView.scrollView.node.height);
            let r = touchMoveVelocity.y / collectionView.scrollView.node.height;
            if (startOffset && Math.abs(r) >= threshold) {
                idx = Math.round(startOffset.y / collectionView.scrollView.node.height) + (r > 0 ? 1 : -1);
            }
...
c2f.ui.YXFlowLayout = YXFlowLayout;
